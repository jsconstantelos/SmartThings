 ...
 */
metadata {
	definition (name: "My FortrezZ Water Meter", namespace: "jsconstantelos", author: "John Constantelos", mnmn: "SmartThingsCommunity", vid: "fc538ccb-5734-396c-b457-f7fb9b5ffc5f") {
		...
		capability "laughbook63613.waterFlow"
        capability "laughbook63613.waterTotalUsed"
        capability "laughbook63613.waterFlowHigh"
        capability "laughbook63613.waterLastUsed"
        capability "laughbook63613.waterUsedHigh"
...
def installed() {
    ...
    sendEvent(name: "waterLastUsed", value: 0, unit: "gals", displayed: false)
    sendEvent(name: "energy", value: 0, displayed: false)
    sendEvent(name: "cumulative", value: 0, displayed: false)
    sendEvent(name: "waterTotalUsed", value: 0, unit: "gals", displayed: false)
    sendEvent(name: "waterFlowHigh", unit: "gpm", value: 0)
    sendEvent(name: "waterUsedHigh", unit: "gals", value: 0)
}
...
def resetMeter() {
    ...
    sendEvent(name: "cumulative", value: 0, displayed: false)
    sendEvent(name: "waterTotalUsed", value: 0, unit: "gals", displayed: false)
    sendEvent(name: "waterFlowHigh", value: 0, unit: "gpm")
    sendEvent(name: "waterUsedHigh", value: 0, unit: "gals")
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd) {
    ...
    if (delta < 0) { //There should never be any negative values
			...
			if (prevCumulative > device.currentState('waterUsedHigh')?.doubleValue) {
                sendEvent(name: "waterUsedHigh", value: String.format("%3.1f",prevCumulative), unit: "gals", displayed: false)
            }
            sendEvent(name: "power", value: delta, displayed: true)
            sendEvent(name: "gpm", value: delta, displayed: true)
            sendEvent(name: "waterFlow", value: delta, unit: "gpm", displayed: true)
            ...
            sendEvent(name: "waterTotalUsed", value: cmd.scaledMeterValue, unit: "gals", displayed: true)
            sendEvent(name: "waterLastUsed", value: String.format("%3.1f",prevCumulative), unit: "gals", displayed: true)
            sendEvent(name: "water", value: "dry", displayed: true)
            sendEvent(name: "alarmState", value: "Normal Operation", displayed: true)
            if (state.debug) log.debug "Data for the new app to display: ${delta}gpm, ${cmd.scaledMeterValue}gals"
            return
    	} else {
            sendEvent(name: "power", value: delta, displayed: true)
            sendEvent(name: "waterFlow", value: delta, unit: "gpm", displayed: true)
            ...
            if (delta > device.currentState('waterFlowHigh')?.doubleValue) {
                sendEvent(name: "waterFlowHigh", value: String.format("%3.1f",delta), unit: "gpm", displayed: true)
            }
