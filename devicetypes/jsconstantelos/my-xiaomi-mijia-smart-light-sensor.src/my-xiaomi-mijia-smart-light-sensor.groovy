 ...
 *
 *  Original device handler code by a4refillpad, adapted for use with Aqara model by bspranger, modifies by jsconstantelos for the Xiaomi Mijia Smart Light Sensor
    ...
    tiles(scale: 2) {
        multiAttributeTile(name:"illuminance", type:"generic", width: 6, height: 4) {
            tileAttribute ("device.illuminance", key: "PRIMARY_CONTROL") {
                attributeState "default", label:'${currentValue} lux',
                    backgroundColors: [
            ...
            tileAttribute("device.battery", key: "SECONDARY_CONTROL") {
                attributeState("default", label:'Battery Level: ${currentValue}%')
            }
        }
        valueTile("lastcheckin", "device.lastCheckin", decoration:"flat", inactiveLabel: false, width: 3, height: 2) {
            state "default", label:'Last Event:\n ${currentValue}'
        }
        valueTile("batteryRuntime", "device.batteryRuntime", inactiveLabel: false, decoration:"flat", width: 3, height: 2) {
             state "batteryRuntime", label:'Battery Changed:\n ${currentValue}'
        }
		main(["illuminance"])
		details(["illuminance", "battery", "lastcheckin", "batteryRuntime"])
	}
