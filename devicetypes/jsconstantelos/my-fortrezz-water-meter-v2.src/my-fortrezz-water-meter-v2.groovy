 ...
 */
metadata {
	definition (name: "My FortrezZ Water Meter v2", namespace: "jsconstantelos", author: "John Constantelos", mnmn: "SmartThingsCommunity", vid: "71915e0e-77f8-3a50-af25-7eab73cf3728") {
        ...
        capability "laughbook63613.waterUsedTotal"
        capability "laughbook63613.waterUsedLast"
        capability "laughbook63613.waterUsedHighest"
...
def installed() {
    ...
    sendEvent(name: "waterFlowHighRate", unit: "gpm", value: 0)
    sendEvent(name: "waterUsedHighest", unit: "gals", value: 0)
}
...
def resetMeter() {
    ...
    sendEvent(name: "cumulative", value: 0, displayed: false)
    sendEvent(name: "waterUsedTotal", value: 0, unit: "gals", displayed: false)
    sendEvent(name: "waterFlowHighRate", value: 0, unit: "gpm")
    sendEvent(name: "waterUsedHighest", value: 0, unit: "gals")
...
def zwaveEvent(physicalgraph.zwave.commands.meterv3.MeterReport cmd) {
    ...
    if (delta < 0) { //There should never be any negative values
			...
			if (prevCumulative > device.currentState('waterUsedHighest')?.doubleValue) {
                sendEvent(name: "waterUsedHighest", value: String.format("%3.1f",prevCumulative), unit: "gals", displayed: false)
            }
            ...
            if (delta > device.currentState('waterFlowHighRate')?.doubleValue) {
                sendEvent(name: "waterFlowHighRate", value: String.format("%3.1f",delta), unit: "gpm", displayed: true)
            }
