...
definition(
    ...
    category: "My Apps",
    parent: "jsconstantelos:Dashboard - Battery Monitor Parent",
...
def pageStatus() {
	...
	return dynamicPage(pageProperties) {
        ...
        if (listLevel1) {
        	section("Batteries needing attention right now") {
            	paragraph listLevel1.trim()
            }
        }

        if (listLevel2) {
            section("Batteries to keep an eye on") {
                paragraph listLevel2.trim()
            }
        }

        if (listLevel3) {
            section("Batteries with acceptable charge") {
